...
# ── Configuration ─────────────────────────────────────────────────────────────
HIDDEN_SIZE          = 64
LEARNING_RATE        = 0.01 # BUG 1 FIX: LEARNING_RATE was 0.005 — too small for this dataset/architecture
EPOCHS               = 600
CONFIDENCE_THRESHOLD = 0.75 # BUG 5 FIX: CONFIDENCE_THRESHOLD was 0.95, that is very high
...
def preprocess(text: str) -> list:
    ...
    return [lemmatizer.lemmatize(tok) for tok in tokens if tok.isalpha()] # BUG 2 FIX: Filter was tok.isdigit()
...
def _one_hot(idx: int, size: int) -> np.ndarray:
    vec = np.zeros((size, 1))
    vec[idx] = 1.0 # BUG 3 FIX: The active index was set to 0.0 instead of 1.0
    return vec
...
class VanillaRNN:
    ...
    def backward(self, probs: np.ndarray, target_idx: int) -> float:
        ...
        for t in reversed(range(n)):
            dtanh  = (1.0 - self._hs[t + 1] ** 2) * d_h # BUG 4 FIX: was written as + instead of −
            d_bh  += dtanh
